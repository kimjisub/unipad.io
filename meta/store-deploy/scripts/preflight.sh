...
resolve_fastlane() {
  ...
  local brewfl="$(brew --prefix 2>/dev/null)/bin/fastlane"
...
case "$PLATFORM" in
  android)
    ...
    grep -E "applicationId|versionCode|versionName" "$DIR/app/build.gradle" | sed 's/^/  /'
  ...
  ios)
    ...
    grep -E "MARKETING_VERSION|CURRENT_PROJECT_VERSION|PRODUCT_BUNDLE_IDENTIFIER = kim.jisub.unipad;" \
      "$DIR/unipad.xcodeproj/project.pbxproj" | sort -u | sed 's/^[[:space:]]*/  /'
